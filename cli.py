...
@click.option('--summarizer', type=click.Choice(['huggingface', 'openai', 'ollama']), default=None, help='Summarization engine')
@click.option('--openai-api-key', help='OpenAI API key (required for openai summarizer)', hide_input=True)
@click.option('--output-type', type=click.Choice(['file', 'confluence', 'both']), multiple=True, help='Output destination: file, confluence, or both (e.g., --output-type file --output-type confluence)')
...
def generate_release_notes(config, jira_url, jira_username, jira_token, version, jql, summarizer, openai_api_key,
    ...
    # Merge CLI options into config, prioritizing CLI over file
    cfg['jira'] = cfg.get('jira', {})
    cfg['jira']['url'] = jira_url or cfg['jira'].get('url', 'https://uat-givaudan.atlassian.net')
    cfg['jira']['username'] = jira_username or cfg['jira'].get('username')
    cfg['jira']['password'] = jira_token or cfg['jira'].get('password')
    cfg['version'] = version or cfg.get('version', 'Test-release-0.1.0')
    cfg['summarizer'] = cfg.get('summarizer', {})
    cfg['summarizer']['type'] = summarizer or cfg['summarizer'].get('type', 'huggingface')
    if cfg['summarizer']['type'] == 'openai':
        cfg['summarizer']['openai_api_key'] = openai_api_key or cfg['summarizer'].get('openai_api_key')
    if jql:
        cfg['jira']['jql'] = jql
    ...
    if cfg['summarizer']['type'] == 'openai' and 'openai_api_key' not in cfg['summarizer']:
    ...
    if 'both' in output_types:
        output_types = ['file', 'confluence']
    else:
        output_types = list(set(output_types))  # Remove duplicates
    cfg['output'] = cfg.get('output', {})
    cfg['output']['type'] = output_types
    cfg['output']['file_path'] = file_path or cfg['output'].get('file_path', f"output/release_notes_{cfg['version']}.{file_format}")
    cfg['output']['format'] = file_format or cfg['output'].get('format', 'markdown')

    if 'confluence' in output_types:
        cfg['output']['confluence'] = cfg['output'].get('confluence', {})
        cfg['output']['confluence']['url'] = confluence_url or cfg['output']['confluence'].get('url', 'https://uat-givaudan.atlassian.net/wiki')
        cfg['output']['confluence']['username'] = confluence_username or cfg['output']['confluence'].get('username')
        cfg['output']['confluence']['api_token'] = confluence_token or cfg['output']['confluence'].get('api_token')
        cfg['output']['confluence']['space_key'] = space_key or cfg['output']['confluence'].get('space_key', 'FP')
        cfg['output']['confluence']['page_title'] = page_title or cfg['output']['confluence'].get('page_title', f"Release Notes - {cfg['version']}")
        cfg['output']['confluence']['parent_page_id'] = parent_page_id or cfg['output']['confluence'].get('parent_page_id')
